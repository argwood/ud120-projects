...
#features_list = ["bonus", "long_term_incentive"] #is long term incentive a better feature to use? -> yes, a bit
...
#what if we swap the test and training set since the latter doesn't have an outlier?
reg.fit(feature_test, target_test)
plt.plot(feature_train, reg.predict(feature_train), color='b')
print('slope of regression on testing set (without outlier): ', reg.coef_)
plt.xlabel(features_list[1])
plt.ylabel(features_list[0])
plt.legend()
plt.savefig('finance_reg.png')
